...
{
	static readonly string sPackagesPath = Path.Combine(
		Application.dataPath,
		"..",
		"Packages"
	);
	static readonly string sManifestJsonPath = Path.Combine(
		sPackagesPath,
		"manifest.json"
	);
	...
	static bool IsUsingRightFileVersion(Manifest manifest, string bundleId)
	{
		...
		return dependencies.Any(
			d =>
				d.Contains(bundleId)
				&& d.Contains("file:")
				&& VersionNumberIsTheLatestTarball(d)
		);
	}
	...
	{
		if (
			string.IsNullOrEmpty(currentVersionString)
			|| string.IsNullOrEmpty(VUFORIA_VERSION)
		)
			return false;
	...
	static void DisplayAddPackageDialogue(
		Manifest manifest,
	...
	static void DisplayDependenciesDialogue(
		Manifest manifest,
		IEnumerable<PackageDescription> packages
	)
	{
	...
	{
		var tarFilePaths = Directory
			.GetFiles(
				Path.Combine(
					Directory.GetCurrentDirectory(),
					VUFORIA_TAR_FILE_DIR
				)
			)
			.Where(f => f.EndsWith(".tgz"));
	...
	class Manifest
	{
		...
		public void JsonSerialize(string path)
		{
			...
			stringBuilder.Append(
				jsonString.Substring(endIndex, jsonString.Length - endIndex)
			);
		...
		string GetJsonString()
		{
			if (ScopedRegistries.Length > 0)
				return JsonUtility.ToJson(
					new UnitySerializableManifest
					{
						scopedRegistries = ScopedRegistries,
						dependencies = new DependencyPlaceholder()
					},
					true
				);

			return JsonUtility.ToJson(
				new UnitySerializableManifestDependenciesOnly()
				{
					dependencies = new DependencyPlaceholder()
				},
				true
		...
		public static Manifest JsonDeserialize(string path)
		{
			...
			var registries =
				JsonUtility
					.FromJson<UnitySerializableManifest>(jsonString)
					.scopedRegistries ?? new ScopedRegistry[0];
			var dependencies = DeserializeDependencies(jsonString);

			return new Manifest
			{
				ScopedRegistries = registries,
		...
		static string DeserializeDependencies(string json)
		{
			...
			var dependencies = json.Substring(
				startIndex,
				endIndex - startIndex
			);
			return dependencies;
		...
		{
			var dependenciesIndex = json.IndexOf(
				DEPENDENCIES_KEY,
			...
			var dependenciesStartIndex = json.IndexOf(
				'{',
				dependenciesIndex + DEPENDENCIES_KEY.Length
			);
